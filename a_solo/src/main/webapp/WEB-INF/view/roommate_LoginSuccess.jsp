		...
		<button type="button" class="btn btn-default" data-toggle="modal" data-target="#logout">Logout</button>
                ...
                <nav id="main_lnb">
                    <ul>
                        ...
                        <li><a href="bbs_list_Roommate_Success.do" id="roommate">룸메찾기</a></li>
...
<c:forEach var="row" items="${headers}">
	<tr><td>${row.num} </td>
		...
		<a href=./bbs_read.do?action=read&num=${row.num}>${row.subject}</a>
		</td>
		<td>${row.writer}</td>
		<td>${row.idate}</td>
...
<%	
	...
	if(totalBlock > currentBlock) { 
		...
		if(togo > totalPage)
			togo = (int) totalPage; %>
		<a href=./bbs_list_Roommate_Success.do?page=<%=togo%>> << </a>
<%	}
	for(int i = numPerBlock; i > 0; i--) {
		...
		if(pn > totalPage)
			continue;
		if(pn == mypage) { %>
		<a href=./bbs_list_Roommate_Success.do?page=<%=pn%>>
		<span style="text-decoration:underline"><%=pn%></span></a>&nbsp;
<%		} else { %>
		<a href=./bbs_list_Roommate_Success.do?page=<%=pn%>><%=pn%></a>&nbsp		
<%		}
	}
	if(currentBlock > 1) { %>
		<a href=./bbs_list_Roommate_Success.do?page=<%= (currentBlock-1)*numPerBlock %>> >> </a>
<%	} %> </div> 
</div>                           
